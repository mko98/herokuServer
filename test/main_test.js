...
describe('Authentication register', function () {
    ...
    it('returns an error on POST /api/v1/register without password', function(done) {
        var user = {
            firstname: "geenwachtwoord",
        };
        chai.request(server)
            .post('/apiv1/register')
            .send(user)
            .end(function(err, res) {
                ...
                res.body.should.have.property('error').equal('Usage: username and password must be submitted');
                done();
            });
    });

    it('returns a msg on POST /api/v1/register with valid credentials', function(done) {
        var d = new Date();
        var n = d.getTime();
        var user = {
            username: "nieuwetestuser",
            password: "nieuwetestuser"
        };
        chai.request(server)
    ...
    it('returns an error on POST /apiv1/login with invalid credentials ', function(done) {
        ...
        chai.request(server)
            ...
            .end(function(err, res) {
                ...
                res.should.be.json;
                res.body.should.be.an('object');
                res.body.should.have.property('error').equal('Invalid credentials, bye');
                done();
            });
    });
...
describe('/filmid test', function(){
    it('returns wrong usage error on GET /api/v1/films/:filmid withoud id query', function(done){
        chai.request(server)
    ...
    it('returns wrong usage error on GET /api/v1/films/1001', function(done){
        chai.request(server)
    ...
    it('returns data on GET /api/v1/films/1', function(done){
        chai.request(server)
            .get('/api/v1/films/1')
            .end(function(err, res){
                res.should.have.status(200);
...
describe('/films test', function () {
    ...
    it('returns wrong usage error on GET /api/v1/films without query', function(done) {
        chai.request(server)
            .get('/api/v1/films')
            .end(function(err, res) {
                res.should.have.status(400);
    ...
    it('returns wrong usage error on GET /api/v1/films?offset=1', function(done) {
        chai.request(server)
    ...
    it('returns wrong usage error on GET /api/v1/films?offset=1&count=0', function(done) {
        chai.request(server)
    ...
    it('returns data on GET /api/v1/films?offset=1&count=10', function(done) {
        chai.request(server)
            .get('/api/v1/films?offset=1&count=10')
            .end(function(err, res) {
                res.should.have.status(200);
                res.should.be.json;
                res.body.should.be.a('object');
